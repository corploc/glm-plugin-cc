...
import { getGlmAvailability, getGlmLoginStatus, runReview, runTask } from "./lib/glm.mjs";
...
function printUsage() {
  console.log([
    "Usage:",
    "  node scripts/glm-companion.mjs setup [--json] [--enable-review-gate|--disable-review-gate]",
    "  node scripts/glm-companion.mjs review [--wait|--background] [--base <ref>] [--scope <auto|working-tree|branch>]",
    "  node scripts/glm-companion.mjs adversarial-review [--wait|--background] [--base <ref>] [--scope <auto|working-tree|branch>] [focus text]",
    "  node scripts/glm-companion.mjs task [--background] [--write] [--model <model>] [prompt]",
...
async function buildSetupReport(cwd) {
  ...
  const zai = await getGlmAvailability();
  const auth = await getGlmLoginStatus();
  const ready = zai.available && auth.loggedIn;
  const config = getConfig(resolveWorkspaceRoot(cwd));

  const nextSteps = [];
  if (!zai.available) {
    nextSteps.push("Install the GLM CLI: npm install -g @guizmo-ai/zai-cli");
  }
...
async function ensureGlmReady() {
  const zai = await getGlmAvailability();
  if (!zai.available) {
    throw new Error("GLM CLI is not installed. Install it with: npm install -g @guizmo-ai/zai-cli");
  }
  const auth = await getGlmLoginStatus();
  if (!auth.loggedIn) {
    throw new Error("GLM CLI is not authenticated. Set ZAI_API_KEY or run `zai config`.");
  }
}
...
function spawnDetachedTaskWorker(cwd, jobId) {
  const scriptPath = path.join(ROOT_DIR, "scripts", "glm-companion.mjs");
...
async function handleTask(argv) {
  ...
  if (options.background) {
    ...
    outputCommandResult(payload, `${title} started in the background as ${payload.jobId}. Check /glm:status ${payload.jobId} for progress.\n`, options.json);
    return;
  }
...
main().then(
  ...
  (error) => {
    if (error?.code === "RATE_LIMITED" || error?.code === "MODEL_UNAVAILABLE" || error?.code === "CLI_ERROR") {
      const lines = [`# GLM Error`, ``, `Model: ${error.model ?? "unknown"}`, `Status: ${error.code}`, ``, error.message];
